...
public class ReplicationConfiguration {
    ...
    private static final String GERRITMS_JGIT_REPLICATION_DISABLED_DEFAULT = "gerritms_jgit_replication_disabled_default";
     ...
     * @throws IOException If application.properties could not be read
     */
    ...
    public static boolean isReplicationDisabled(){
        if (replicationDisabled == null) {
          Boolean defaultBehaviour = null;

          if (System.getProperties().containsKey(GERRITMS_JGIT_REPLICATION_DISABLED_DEFAULT)) {
            defaultBehaviour = Boolean.valueOf(System.getProperty(GERRITMS_JGIT_REPLICATION_DISABLED_DEFAULT));
          }
          replicationDisabled = getOverrideBehaviour(REPLICATION_DISABLED, defaultBehaviour);
        }
    ...
    public static boolean isReplicatedRepo(Repository repository) {
        ...
        return config.getBoolean("core", "replicated", false); //$NON-NLS-1$
    }


    /**
     * Search for a parameter in a .git/config
     * In certain contexts, a git repository without a replicated flag means something different to a git repository
     * with a replicated = false flag. This method is used to detect the presence of a "replicated"
     * key/value but can be used more generally to look for any key/value pair in a .git/config.
     * @param keyToLookFor A key to search for within a .git/config
     * @param repository ( Git Repository in which to check the config )
     * @return True if the replicated parameter is present.
     */
    public static String getRepositoryCoreConfigKey(final String keyToLookFor, Repository repository){
        StoredConfig config = repository.getConfig();
        return config.getString("core", null, keyToLookFor);
    }
     ...
     *
     * @param overrideName Name of Environment variable / System property
     * @return (Default:False)
    ...
    private static boolean loadApplicationProperties() throws IOException {
        ...
        if (!tmpAppPropertiesFile.exists()) {
            throw new IOException("Failed to find replication configuration " +
                    "file - application.properties, check the gitmsconfig " +
                    "path set in .gitconfig.");//$NON-NLS-1$
        }

        if (!tmpAppPropertiesFile.canRead()) {
            throw new IOException("Failed to read application.properties, " +
                    "check permissions on the replication configuration file " +
